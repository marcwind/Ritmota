...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\Ritmota2 Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\Ritmota2 Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\Ritmota2 manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\Ritmota2.vst3\\Contents\\x86_64-win\\Ritmota2.vst3",
      projectpath + "\\build-win\\Ritmota2_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Ritmota2-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Ritmota2-app_x64.pdb"  
  ]
