...
if %DEMO_ARG% == "demo" (
  echo Making Ritmota2 Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making Ritmota2 Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild Ritmota2-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild Ritmota2.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild Ritmota2.sln /t:Ritmota2-app;Ritmota2-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Ritmota2.aaxplugin\Contents\Win32\Ritmota2.aaxplugin --out .\build-win\aax\bin\Ritmota2.aaxplugin\Contents\Win32\Ritmota2.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Ritmota2.aaxplugin\Contents\x64\Ritmota2.aaxplugin --out .\build-win\aax\bin\Ritmota2.aaxplugin\Contents\x64\Ritmota2.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\Ritmota2.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\Ritmota2.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Ritmota2 Installer" ".\installer\Ritmota2 Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\Ritmota2 Installer.exe" ".\installer\Ritmota2 Demo Installer.exe"
  REM -del ".\installer\Ritmota2 Installer.exe"
  REM -)
